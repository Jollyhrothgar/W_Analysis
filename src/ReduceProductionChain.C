#include "ReduceProductionChain.h"
...
int ReduceProductionChain(TChain * ch, std::string outTree = "" )
{
  ...
  for(Long64_t i = 0; i < entries; i++)
  {
    ...
    for( unsigned int Track_i = 0; Track_i < static_cast<unsigned int>(sm._RecoTracks); Track_i++)
    {
      ...
      if( sm.Evt_Nmu[Track_i] != 1               ) basicTrackCut["Evt_Nmu"] += 1.0;
    ...
    for( unsigned int Track_i = 0; Track_i < static_cast<unsigned int>(sm._RecoTracks); Track_i++)
    {
      ...
      if( sm.Evt_Nmu[Track_i] != 1               ) continue;
